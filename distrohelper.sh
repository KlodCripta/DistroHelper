...
# MODIFICA 4: limita i motivi mostrati ai primi 3
get_top_reasons() {
  local raw="$1"
  local IFS=';'
  read -ra parts <<< "$raw"
...
# MODIFICA 1: domanda live spostata qui (prima era la 20)
# Questa posizione garantisce che live_enabled sia noto
# prima delle domande 10, 11 e seguenti che la usano
...
# 18 — MODIFICA 3: pesi ridotti di 1 (era 4→3, 3→2)
...
q18=$(ask_yes_no)
if [[ "$q18" == "s" ]]; then
  add_many 3 "vuoi software molto aggiornato" "Arch_Linux" "Endeavour_OS" "CachyOS" "Garuda_Linux" "Manjaro"
  add_many 2 "apprezzi un sistema moderno" "Fedora" "Solus"
...
# 24 — MODIFICA 3: pesi ridotti di 1 (era 4→3, 3→2)
print_question_header 24
print_question "Ti interessa avere il massimo controllo sul sistema operativo?"
q24=$(ask_yes_no)
if [[ "$q24" == "s" ]]; then
  add_many 3 "vuoi grande controllo sul sistema" "Arch_Linux" "Gentoo" "Slackware" "Alpine_Linux" "Venom_Linux"
  add_score "Endeavour_OS" 2 "puoi apprezzare controllo e flessibilità"
else
  add_many 2 "preferisci una gestione più semplice" "Ubuntu" "Linux_Mint" "Zorin_OS" "Pop_OS"
fi
...
for line in "${top3[@]}"; do
  ...
  echo "   Affinità: ${percent}%"     # MODIFICA 2: rinominato da "Compatibilità" ad "Affinità"
  echo "   Punteggio: $pts"
  echo "   ${descriptions[$distro]}"
  if [[ -n "${reasons[$distro]}" ]]; then
    top_reasons=$(get_top_reasons "${reasons[$distro]}")   # MODIFICA 4: solo i primi 3 motivi
    echo "   Motivi principali: ${top_reasons}"
  fi
  echo ""
  ((rank++))
done
